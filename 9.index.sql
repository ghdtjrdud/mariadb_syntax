...
select * from author where id=1 and name = "honggildong"l

-- 만약에 두컬럼에 동시에 index가 걸려있다면 해당 index활용
-- 만약에 두컬럼 각각 index가 설정되어있다면 mariadb엔진에서 최적의 알고리즘 실행
select * from author where name = "honggildong" and address = '서울시';

-- index 제거
alter table author drop index 인덱스명;

--  대량 데이터 생성 프로시저 생성 후 조회
DELIMITER //
